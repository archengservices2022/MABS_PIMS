...
class JobFormTab(QtWidgets.QWidget):
    ...
    def show_actions_context_menu(self, pos, job_data):
        ...
        email_action = menu.addAction("📧 Email to Client")
        email_action.triggered.connect(lambda: self.email_quote_to_client(job_data))
    ...
    def create_job_forms_table_section(self, layout):
        ...
        # ===== NEW: Date Range Filter Button (LEFT SIDE) =====
        ...
        self.status_filter_combo.addItems(["All Status", "Not Started", "On Hold", "Converted", "Completed", "Cancel", "Urgent", "High", "Medium", "Low"])
        ...
        self.job_forms_table.setColumnCount(8)  # Removed Start Date and Due Date
        self.job_forms_table.setHorizontalHeaderLabels([
            "Quote Number", "Project Name", "Sales", "Client", 
            "Cost", "Scope of Work", "Status", "Actions"
        ])
    ...
    """)
        ...
        # Keep key columns fixed and let descriptive columns absorb available width.
        for col in range(self.job_forms_table.columnCount()):
            header.setSectionResizeMode(col, QtWidgets.QHeaderView.Fixed)
        header.setSectionResizeMode(1, QtWidgets.QHeaderView.Stretch)  # Project Name
        header.setSectionResizeMode(5, QtWidgets.QHeaderView.Stretch)  # Scope of Work - ensure full visibility
        header.setSectionResizeMode(7, QtWidgets.QHeaderView.Fixed)   # Actions - fixed width

        # Column widths — Optimized to prevent status column overlap
        self.job_forms_table.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarAsNeeded)
        self.job_forms_table.setColumnWidth(0, 95)    # Quote Number
        self.job_forms_table.setColumnWidth(2, 120)   # Sales (full names)
        self.job_forms_table.setColumnWidth(3, 240)   # Client (full names like "Arch engineering services,LLC")
        self.job_forms_table.setColumnWidth(4, 75)    # Cost
        self.job_forms_table.setColumnWidth(5, 280)   # Scope of Work (expanded for full descriptions)
        self.job_forms_table.setColumnWidth(6, 120)   # Status badge (increased to prevent overlap)
        self.job_forms_table.setColumnWidth(7, 240)   # Actions (reduced to make space)
        self.job_forms_table.verticalHeader().setDefaultSectionSize(70)  # Accommodate action buttons
        self.job_forms_table.setWordWrap(True)  # Enable word wrap for full content display
        self.job_forms_table.setTextElideMode(QtCore.Qt.ElideNone)  # Don't truncate text with ...
...
def display_filtered_forms(self, forms):
        ...
        for row, job in enumerate(forms):
            # Quote Number - Compact professional style
            job_number_item = QtWidgets.QTableWidgetItem(job['job_number'])
            job_number_item.setForeground(QtGui.QColor('#2c3e50'))
            job_number_item.setFont(QtGui.QFont("Inter", 13))
            job_number_item.setTextAlignment(QtCore.Qt.AlignCenter)  # Explicit center alignment
            ...
            project_name_item.setFont(QtGui.QFont("Inter", 12))
            project_name_item.setTextAlignment(QtCore.Qt.AlignCenter)
            if age_str:
                project_name_item.setToolTip(f"{project_name} — created {age_str}")
            self.job_forms_table.setItem(row, 1, project_name_item)

            # Sales column
            sales_val = job.get("sales", "")
            sales_item = QtWidgets.QTableWidgetItem(sales_val)
            sales_item.setFont(QtGui.QFont("Inter", 13))
            sales_item.setForeground(QtGui.QColor("#2c3e50"))
            sales_item.setTextAlignment(QtCore.Qt.AlignCenter)
            self.job_forms_table.setItem(row, 2, sales_item)

            
            # Client - Compact
            client_item = QtWidgets.QTableWidgetItem(job['client'])
            client_item.setForeground(QtGui.QColor('#34495e'))
            client_item.setFont(QtGui.QFont("Inter", 13))
            client_item.setTextAlignment(QtCore.Qt.AlignCenter)  # Explicit center alignment
            self.job_forms_table.setItem(row, 3, client_item)
            
            # Cost - Professional financial styling
            cost = job.get('engineering_costs', 'N/A')
            cost_item = QtWidgets.QTableWidgetItem(cost)
            if cost != 'N/A' and '$' in str(cost):
                cost_item.setForeground(QtGui.QColor('#27ae60'))
                cost_item.setFont(QtGui.QFont("Inter", 13))
            else:
                cost_item.setForeground(QtGui.QColor('#95a5a6'))
            cost_item.setTextAlignment(QtCore.Qt.AlignCenter)
            self.job_forms_table.setItem(row, 4, cost_item)
            
            # Scope of Work - Full text display with wrapping
            scope = job.get('scope_of_work', 'N/A')
            scope_item = QtWidgets.QTableWidgetItem(scope)
            scope_item.setForeground(QtGui.QColor('#7f8c8d'))
            scope_item.setFont(QtGui.QFont("Inter", 12))
            scope_item.setTextAlignment(QtCore.Qt.AlignLeft | QtCore.Qt.AlignVCenter)  # Left align for better readability
            scope_item.setFlags(QtCore.Qt.ItemIsSelectable | QtCore.Qt.ItemIsEnabled)  # Allow text wrapping
            self.job_forms_table.setItem(row, 5, scope_item)
            
            # Status pill badge — click to change
            current_status = job.get('status', 'Not Started')
            status_badge = self.create_status_badge(current_status, job, row)
            self.job_forms_table.setCellWidget(row, 6, status_badge)
            
            # Enhanced action buttons - visible and professional
            self.add_enhanced_action_buttons(row, job)
        
        # Set row height to accommodate action buttons
        for row in range(self.job_forms_table.rowCount()):
            self.job_forms_table.setRowHeight(row, 70)
    ...
    # ── status palette: (background, text, border) ───────────────────────────
        ...
        "Converted":   ("#ede9fe", "#4c1d95", "#c4b5fd"),  # purple — sent to invoice
        "Expired":     ("#fee2e2", "#7f1d1d", "#fca5a5"),  # dark red — past due
        ...
        lay.setContentsMargins(5, 2, 5, 2)  # Add margins to prevent overlap
        lay.setSpacing(0)
        lay.setAlignment(QtCore.Qt.AlignCenter)

        badge = QtWidgets.QPushButton(f"{status} ▾")
        badge.setFixedSize(92, 28)
        ...
        def show_status_menu(checked=False, b=badge, j=job_data, r=row):
            ...
            # Position menu to ensure it's visible on screen
            global_pos = b.mapToGlobal(QtCore.QPoint(0, b.height()))
            screen = QtWidgets.QApplication.desktop().screenGeometry()
            
            # Adjust position if menu would go off screen
            if global_pos.y() + 200 > screen.bottom():  # Estimate menu height
                # Show menu above button instead
                global_pos = b.mapToGlobal(QtCore.QPoint(0, -200))
            
            # Adjust horizontal position if needed
            if global_pos.x() + 150 > screen.right():  # Estimate menu width
                global_pos.setX(screen.right() - 150)
    ...
    def add_enhanced_action_buttons(self, row, job_data):
        """Add professional action buttons with View, PDF, and 3-dots menu - UPDATED to match invoice history"""
        # Create container widget for buttons
        actions_widget = QtWidgets.QWidget()
        actions_layout = QtWidgets.QHBoxLayout(actions_widget)
        actions_layout.setContentsMargins(3, 5, 3, 5)
        actions_layout.setSpacing(3)
        actions_layout.setAlignment(QtCore.Qt.AlignVCenter | QtCore.Qt.AlignCenter)

        BTN_H = 32  # Increased button height

        def _mk_btn(label, bg, fg, hover_bg):
            b = QtWidgets.QPushButton(label)
            b.setFixedHeight(BTN_H)
            b.setMinimumWidth(65)  # Increased button width
            b.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
            b.setStyleSheet(f"""
                QPushButton {{
                    background: {bg};
        ...
        view_btn = _mk_btn("View", "#DBEAFE", "#1D4ED8", "#BFDBFE")
        view_btn.setToolTip("View Details")
        view_btn.clicked.connect(lambda: self.view_job_details(job_data))

        pdf_btn = _mk_btn("PDF", "#D1FAE5", "#065F46", "#A7F3D0")
        pdf_btn.setToolTip("Open Generated PDF")
        pdf_btn.clicked.connect(lambda: self.open_job_form_pdf(job_data))

        more_btn = _mk_btn("⋯", "#F1F5F9", "#475569", "#E2E8F0")
        more_btn.setText("More")
        more_btn.setFixedWidth(50)
        more_btn.setToolTip("More Actions")

        inv_btn = _mk_btn("Project", "#FEF3C7", "#92400E", "#FDE68A")
        inv_btn.setToolTip("Create Project from this Quote")

        # 🔐 CHECK USER ROLE - disable Project button for sales users
        main_window = getattr(self, 'main_window', None)
        if main_window and hasattr(main_window, 'current_role') and main_window.current_role == "sales":
            inv_btn.setEnabled(False)
            inv_btn.setToolTip("Sales users cannot create projects. Contact an administrator.")
        ...
        def _on_invoice_click(checked=False, jd=job_data):
            # Enhanced conversion with intelligent auto-population
            try:
                enhanced_job_data = self._prepare_enhanced_project_data(jd)
                print(f"DEBUG: Converting quote to project: {enhanced_job_data.get('job_number', 'N/A')}")
                self.convert_to_project.emit(enhanced_job_data)
                print(f"DEBUG: Project conversion signal emitted successfully")
            except Exception as e:
                print(f"ERROR: Failed to convert quote to project: {e}")
        ...
        inv_btn.clicked.connect(_on_invoice_click)

        def _show_more(checked=False, b=more_btn, jd=job_data):
            menu = QtWidgets.QMenu(b)
            menu.setStyleSheet("""
                QMenu {
                    background: #ffffff;
            ...
            # For sales users, hide certain menu items
            copy_action = QtWidgets.QAction("Copy Quote Number", menu)
            copy_action.triggered.connect(lambda: self.copy_job_number(jd))
            menu.addAction(copy_action)
            
            edit_action = QtWidgets.QAction("Edit Quote Form", menu)
            edit_action.triggered.connect(lambda: self.edit_job_form(jd))
            menu.addAction(edit_action)
            
            # 🔐 For sales users, disable Email to Client? (optional - keep enabled)
            email_action = QtWidgets.QAction("Email to Client", menu)
            email_action.triggered.connect(lambda: self.email_quote_to_client(jd))
        ...
        more_btn.clicked.connect(_show_more)

        actions_layout.addWidget(view_btn)
        actions_layout.addWidget(pdf_btn)
        actions_layout.addWidget(inv_btn)
    ...
    def email_quote_to_client(self, job_data: dict):
        ...
        # Look for the PDF in the temp folder or local backup
        import tempfile
        job_number = job_data.get('job_number', '')
        pdf_candidates = [
            Path(tempfile.gettempdir()) / "mabs_jobforms_temp" / f"{job_number}_job_form.pdf",
    ...
    def apply_client_filter(self, company_name):
        ...
        # Update the client filter button text based on selection
        if company_name == "All Clients" or company_name == "📂 All Clients":
            ...
            # Show the client name on the button (truncated if too long)
            # Remove any emoji prefix from the company name
            clean_company_name = company_name
            if company_name.startswith("🏢 "):
                clean_company_name = company_name[2:].split(" (")[0]  # Remove the "🏢 " prefix
            
            display_name = clean_company_name[:15] + "..." if len(clean_company_name) > 15 else clean_company_name
            ...
            """)
            self.client_filter_button.setToolTip(f"Client: {clean_company_name}\nClick to change")
    ...
    def filter_job_forms(self):
        """Filter quote forms based on search criteria, status filter, client filter, and date range"""
        client_filter = self.selected_client_filter
        search_text = self.search_edit.text().lower()
        status_filter = self.status_filter_combo.currentText()
        
        # Normalize client filter (remove emoji prefix if present)
        ...
        for job in self.job_forms:
            ...
            # Date range filter — uses start_date (MM-dd-yyyy)
            matches_date = True
            if date_range_active and from_date and to_date:
                try:
                    job_date_str = job.get('start_date', '')
            ...
            # Apply ALL filters with AND logic (all must be true)
            include_job = (
                matches_client and 
                matches_search and 
                matches_status and 
        ...
        # Update stats cards based on filtered forms
        self.update_filtered_stats(filtered_forms)
        
        # ✅ UPDATE CLIENT FILTER MENU WITH CURRENTLY FILTERED CLIENTS
        self.update_client_filter_menu()
...
class JobFormDialog(QtWidgets.QDialog):
    ...
    def init_ui(self):
        ...
        # Client field with Intelligence
        client_layout = QtWidgets.QVBoxLayout()
        client_label = QtWidgets.QLabel("Client:")
        client_label.setStyleSheet("font-weight: 500; color: #2c3e50; min-width: 150px;")
        ...
        line = self.client_combo.lineEdit()
        ...
        self.client_intel_btn = QtWidgets.QPushButton("🧠")
        self.client_intel_btn.setFixedSize(32, 32)
        self.client_intel_btn.setStyleSheet("""\
            QPushButton {
                background: #17a2b8;
                color: white;
                border: none;
        ...
        """)
        self.client_intel_btn.clicked.connect(self.toggle_client_intelligence)
        self.client_intel_btn.setToolTip("Show client intelligence and suggestions")
        client_row.addWidget(self.client_intel_btn)
        
        client_layout.addLayout(client_row)
        
        # Client suggestion widget
        self.client_suggestion_widget = ClientSuggestionWidget(self.client_intelligence)
        self.client_suggestion_widget.setVisible(False)
        client_layout.addWidget(self.client_suggestion_widget)
        ...
        scope_label = QtWidgets.QLabel("Scope of Work:")
        scope_label.setStyleSheet("font-weight: 500; color: #2c3e50; min-width: 150px;")
        scope_layout.addWidget(scope_label)
        
        self.scope_of_work_edit = self.create_styled_line_edit("Enter detailed scope of work (comma-separated)...")
        scope_layout.addWidget(self.scope_of_work_edit, 1)
        
        # Template button
        self.template_btn = QtWidgets.QPushButton("📋 Use Template")
        self.template_btn.setFixedSize(100, 32)
        ...
        scope_layout.addWidget(self.template_btn)
    ...
    def on_expedite_clicked(self, checked=None):
        ...
        if checked and hasattr(self, '_last_expedite_btn') and self._last_expedite_btn == sender:
            ...
            # Regular selection - ensure the other button is unchecked
        ...
        # Update the expedite amount field
        self.update_expedite_amount()
    def update_expedite_amount(self):
        ...
        try:
            ...
            # If any error, set appropriate text
            if self.expedite_yes.isChecked():
    ...
    def open_generated_pdf_and_close(self, job_data, dialog):
        ...
        try:
            ...
            if success:
                ...
                if pdf_success:
                    ...
                    """)
                    open_pdf_btn.clicked.connect(lambda: self.open_generated_pdf_and_close(job_data, confirm_dialog))
    ...
    def generate_job_form_pdf(self, job_data):
        ...
        try:
            ...
            left_section = [
                ...
                Table(
                    [[Paragraph(
                        f"<para align='left' leading='12'>"  # Reduced from 12
                        f"<b>Payable to:</b> {company_name}<br/>"
                        f"<b>Mailing Address:</b><br/>15455 Manchester Rd, PO Box 1144, Manchester, MO 63011"
                        f"</para>",
                        styles['FieldValue']
                    )]],
                    colWidths=[available_width * 0.60],
    ...
    def validate_form(self):
        """Validate required fields before saving. Highlights the first missing field."""

        _REQUIRED_BORDER = "border: 2px solid #e74c3c;"
        _NORMAL_BORDER   = ""

        def _require(widget, label: str, error_msg: str) -> bool:
            """Return True if widget has a value, otherwise highlight it and warn."""
            ...
            if not value:
                ...
                widget.setStyleSheet(orig + _REQUIRED_BORDER)
                # Remove highlight once the user types
                def _clear(w=widget, s=orig):
                    w.setStyleSheet(s)
                if isinstance(widget, QtWidgets.QComboBox):
                    widget.currentTextChanged.connect(lambda _: _clear())
                else:
                    widget.textChanged.connect(lambda _: _clear())
        ...
        # 1. Quote Number (auto-generated but could still be blank on first open)
        if not _require(self.job_number_edit, "Quote Number",
                        "Please enter a project name so the quote number can be generated."):
            return False

        # 2. Project Name
        if not _require(self.project_name_edit, "Project Name",
                        "A project name is needed to identify this quote."):
            return False

        # 3. Client
        if not _require(self.client_combo, "Client",
                        "Select or type the client/company name for this quote."):
            return False

        # 4. Scope of Work
        if not _require(self.scope_of_work_edit, "Scope of Work",
                        "Describe what work is included in this quote."):
            return False

        # 5. Agreed Cost — must be present and numeric > 0
        cost_text = self.engineering_costs_edit.text().strip().replace("$", "").replace(",", "")
        if not cost_text:
            self.engineering_costs_edit.setStyleSheet(
                self.engineering_costs_edit.styleSheet() + _REQUIRED_BORDER)
            self.engineering_costs_edit.textChanged.connect(
                lambda _: self.engineering_costs_edit.setStyleSheet(
                    self.engineering_costs_edit.styleSheet().replace(_REQUIRED_BORDER, "")))
            QtWidgets.QMessageBox.warning(
                self, "Required Field",
                "Agreed Cost is required.\nEnter the engineering cost for this quote."
            )
            self.engineering_costs_edit.setFocus()
            return False

        try:
            if float(cost_text) < 0:
                raise ValueError
        except ValueError:
            QtWidgets.QMessageBox.warning(
                self, "Invalid Cost",
                f"'{self.engineering_costs_edit.text()}' is not a valid cost.\nEnter a number (e.g. 1500 or 1500.00)."
            )
            self.engineering_costs_edit.setFocus()
            return False

        # 6. At least one service must be selected
        service_checks = [
            self.structural_checkbox, self.civil_checkbox, self.electrical_checkbox,
            self.mechanical_checkbox, self.plumbing_checkbox, self.anchor_calc_checkbox,
            self.solidworks_checkbox, self.foundation_checkbox, self.other_checkbox,
        ]
        if not any(c.isChecked() for c in service_checks):
            QtWidgets.QMessageBox.warning(
                self, "Service Required",
                "Please select at least one Service Required\n"
                "(e.g. Structural, Civil, Electrical…)"
            )
            self.structural_checkbox.setFocus()
            return False

...
def invoicing():
    ...
    for iid, idata in (raw.items() if isinstance(raw, dict) else []):
        if idata and isinstance(idata, dict):
            idata["firebase_id"] = iid
            # Get all project numbers from line items
            project_numbers = set()
            for li in (idata.get("line_items") or []):
                if isinstance(li, dict) and li.get("project_number"):
                    project_numbers.add(li["project_number"])

            # Fallback to meta project_number if no line items
            if not project_numbers:
                meta = idata.get("meta", {}) or {}
                proj_num = meta.get("project_number", "")
                if proj_num:
                    project_numbers.add(proj_num)

            # Get plants for all projects
            plants = set()
            for pnum in project_numbers:
                plant = proj_plant_map.get(pnum, "")
                if plant:
                    plants.add(plant)

            # Show plant if:
            # 1. Only one project in the invoice, OR
            # 2. Multiple projects but all have the same plant
            if len(project_numbers) == 1 and len(plants) == 1:
                # Single project: show its plant
                idata["plant_state"] = plants.pop()
            elif len(project_numbers) > 1 and len(plants) == 1:
                # Multiple projects with same plant: show that plant
                idata["plant_state"] = plants.pop()
            else:
                # Multiple projects with different plants: show blank
                idata["plant_state"] = ""
...
@app.route("/clients/<client_name>/delete", methods=["GET"])
@role_required("invoicing")
...
def financial():
    ...
    # Recalculate status for each invoice based on actual payments
    ...
    for r in rev_list:
        ...
        # Get current amount paid from invoice meta (not stale revenue entry)
        amount_paid = _safe_float(inv_meta.get("amount_paid", 0))
        tax_paid = sum(_safe_float(tp.get("amount", 0)) for tp in (inv_data.get("tax_payments", []) or []))
        total_paid = amount_paid + tax_paid

        # Update the revenue entry with current amounts from the invoice
        r["amount_paid"] = amount_paid
        r["total"] = inv_total
        r["tax_amount"] = _safe_float(inv_meta.get("tax_amount", 0))

        # Only include invoices that have SOME payment (not unpaid invoices)
        if total_paid <= 0:
            continue
    ...
    # Total collected = amount_paid + tax_paid (same as Invoicing tab)
    # Only count invoices in selected year and that are not deleted
    total_collected = 0.0
    for r in rev_list:
        if _extract_year_from_date(r.get("date", "")) == stat_card_year and r.get("invoice_id") in invoices:
            # Add base amount paid
            total_collected += _safe_float(r.get("amount_paid", 0))
            # Add tax payments from the actual invoice (only those from selected year)
            inv_id = r.get("invoice_id")
            if inv_id and inv_id in invoices:
                inv_data = invoices[inv_id]
                if isinstance(inv_data, dict):
                    # Only include tax payments from selected year
                    tax_paid = sum(_safe_float(tp.get("amount", 0))
                                  for tp in (inv_data.get("tax_payments", []) or [])
                                  if _extract_year_from_date(tp.get("date", "")) == stat_card_year)
                    total_collected += tax_paid
    ...
    total_outstanding = total_invoiced - total_paid
    # Filter expenses by selected year for stat cards
    exp_list_filtered = [e for e in exp_list if _extract_year_from_date(e.get("date", "")) == stat_card_year]
    total_expenses    = sum(_safe_float(e.get("amount", 0)) for e in exp_list_filtered)
    ...
    # Build annual revenue based on PAYMENT DATES (not invoice dates)
    for inv in inv_list:
        # Process regular payments from payment_log
        for pay in (inv.get("payment_log", []) or []):
            ...
            try:
                ...
                if pay_date.year == current_year:
                    ...
                    annual_revenue[month] += _safe_float(pay.get("amount", 0))
            except Exception:
                pass
        ...
        for tax_pay in (inv.get("tax_payments", []) or []):
            ...
            try:
                ...
                if tax_date.year == current_year:
                    ...
                    annual_revenue[month] += _safe_float(tax_pay.get("amount", 0))
            except Exception:
                pass
    ...
    for p in projects_list:
        ...
        # Calculate INVOICED and COLLECTED using the exact same P&L formula
        # This ensures By Project matches the Project Detail P&L exactly
        raw_inv = fb_get("/invoices") or {}
        p_invoiced = 0
        p_collected = 0
        ...
        if isinstance(raw_inv, dict):
            for inv_id, inv_data in raw_inv.items():
                ...
                # Check if this project is in this invoice (using same function as P&L)
                if pnum not in _invoice_linked_projects(inv_data):
                    continue

                inv_meta = inv_data.get("meta", {}) or {}

                # Get project's share (using exact same function as P&L)
                project_share = _invoice_project_share(inv_data, pnum)

                # Calculate invoiced: (total or subtotal+tax) * share (exact P&L formula)
                inv_total = _safe_float(inv_meta.get("total", 0)) or (_safe_float(inv_meta.get("subtotal", 0)) + _safe_float(inv_meta.get("tax_amount", 0)))
                p_invoiced += inv_total * project_share

                # Calculate collected: (amount_paid + tax_payments) * share (exact P&L formula)
                amount_paid = _safe_float(inv_meta.get("amount_paid", 0))
                tax_payments = inv_data.get("tax_payments", []) or []
                tax_paid = sum(_safe_float(tp.get("amount", 0)) for tp in tax_payments)
                p_collected += (amount_paid + tax_paid) * project_share
    ...
    # Calculate totals from project_pnl (sum of all projects)
    # This ensures the header totals match the sum of displayed rows
    project_total_invoiced = sum(_safe_float(p.get("invoiced", 0)) for p in project_pnl)
    project_total_collected = sum(_safe_float(p.get("paid", 0)) for p in project_pnl)
    project_total_expenses = sum(_safe_float(p.get("expenses", 0)) for p in project_pnl)
    project_gross_profit = project_total_collected - project_total_expenses
    ...
    for _inv in inv_list:
        ...
        _inv_status = _inv_meta.get("status", "")
        if _inv_status in ("Draft", "Cancelled"):
            continue
        ...
        for _pay in (_inv.get("payment_log", []) or []):
            ...
            try:
                ...
                if _pay_dt.year == selected_year:
                    ...
                    if not _stage and _proj_num:
                        try:
                            _proj_data = _proj_num_to_data.get(_proj_num, {})
                            _p_stages = _proj_data.get("payment_stages", [])
                            if isinstance(_p_stages, list):
                                # First try: match by invoice_id (same as invoice_detail enrichment)
                                for _pstage in _p_stages:
                                    if isinstance(_pstage, dict) and _pstage.get("invoice_id") == _inv_id:
                                        _stage = _pstage.get("name", "")
                                        break

                                # Second try: use stage_index if available
                                if not _stage:
    ...
    # Calculate total_salaries AFTER filtering by year
    ...
    for _bucket in aging_buckets.values():
        for _entry in _bucket:
            # By due date - use this for outstanding invoices (filtered by month user selects)
            _ds2 = (_entry.get("due_date") or "")[:10]
            try:
                _d2 = datetime.fromisoformat(_ds2)
                if _d2.year == selected_year:
                    monthly_outstanding_details[str(_d2.month)].append(_entry)
    ...
    return render_template("financial.html",
        ...
        project_total_invoiced=project_total_invoiced,
        project_total_collected=project_total_collected,
        project_total_expenses=project_total_expenses,
        project_gross_profit=project_gross_profit,
...
def export_balance_sheet():
    ...
    try:
        ...
        # Same calculation as Annual Financial Summary table
                    ...
                    for pay in (inv_data.get("payment_log", []) or []):
                        ...
                        try:
                            ...
                            if pay_date.year == year:
                                month = pay_date.month
                                monthly_revenue[month-1] += _safe_float(pay.get("amount", 0))
                        except Exception:
                            pass
                    ...
                    for tax_pay in (inv_data.get("tax_payments", []) or []):
                        ...
                        try:
                            ...
                            if tax_date.year == year:
                                month = tax_date.month
                                monthly_revenue[month-1] += _safe_float(tax_pay.get("amount", 0))
                        except Exception:
                            pass
...
def invoice_pdf(invoice_id):
    ...
    # Define custom styles exactly matching reference software
    ...
    # Create centered header text with company name size 16 and rest size 9
    ...
    if logo_img:
        # Logo on left, centered header on right
    ...
    # --- Invoice Header Table (clean label + value format) ---
    client_name = meta.get('client_name','')
    client_email = ""
    client_address = ""

    # Fetch client data from clients collection using client_name as key
    if client_name:
        try:
            client_data = fb_get(f"/clients/{client_name}") or {}
    ...
    # Build Bill To text with name, email, and address
    bill_to_lines = []
    if client_name:
    ...
    # Headers: Project, Project Name, Plant, Qty, Unit Price, Payment Stage, Payment Due
    ...
    item_data = [[Paragraph(header, center_bold_style) for header in ["Project", "Project Name", "Plant", "Qty", "Unit Price", "Payment Stage", "Payment Due"]]]
    ...
    for idx, item in enumerate(invoice.get("line_items", [])):
        ...
        # Get project number and project name from linked_projects (same as invoice details)
        project_number = ""
        project_name = ""
        plant = ""

        # Get from linked_projects first
        ...
        # Fallback: get project_number from meta
        if not project_number:
            project_number = meta.get("project_number", "")

        # Fetch project name and plant from database by searching for project_number
        ...
        # If still no plant, try to get it from meta
        if not plant:
            plant = meta.get("plant", "")

        # Last resort: extract from description if project_name still empty
        if not project_name:
            description = item.get("description", "")
            if "—" in description:
        ...
        # Get payment stage from linked_projects or meta (same as payment history)
        payment_stage = ""

        # First try: get from linked_projects payment_stage_index
        if idx < len(linked_projects) and isinstance(linked_projects[idx], dict):
            payment_stage_index = linked_projects[idx].get("payment_stage_index")
            if payment_stage_index is not None and project_number:
                try:
                    # Search for project with matching project_number
                    raw_proj = fb_get("/projects") or {}
                    for pid, pdata in (raw_proj.items() if isinstance(raw_proj, dict) else []):
                        if isinstance(pdata, dict) and pdata.get("project_number") == project_number:
        ...
        # Second try: get from meta.payment_stage
        if not payment_stage:
            payment_stage = meta.get("payment_stage", "")

        # Clean up payment stage: remove "of X" suffix (e.g., "Installment 3 of 4" -> "Installment 3")
        if payment_stage and " of " in payment_stage:
            payment_stage = payment_stage.split(" of ")[0].strip()

        # Default to "Final Payment" if nothing found
        if not payment_stage:
            payment_stage = "Final Payment"

        # Calculate payment due as Qty * Unit Price
        payment_due_val = qty_val * unit_price_val
        payment_due = f"${payment_due_val:,.2f}"

        item_data.append([
            Paragraph(project_number, center_style),  # Project
            Paragraph(project_name, center_style),  # Project Name
            Paragraph(plant or "", center_style),  # Plant
            Paragraph(qty, center_style),  # Qty
            Paragraph(unit_price, center_style),  # Unit Price
            Paragraph(payment_stage, center_style),  # Payment Stage
            Paragraph(payment_due, center_style)  # Payment Due
        ])
    ...
    totals_data = []
    totals_data.append([Paragraph("Total", styles['Right10']), Paragraph(f"${subtotal:,.2f}", styles['Right10'])])
    totals_data.append([Paragraph(f"Tax ({tax_rate}%)", styles['Right10']), Paragraph(f"${tax_amount:,.2f}", styles['Right10'])])
    totals_data.append([Paragraph("TOTAL AMOUNT DUE:", styles['RightBold10']), Paragraph(f"${total_amount:,.2f}", styles['RightBold10'])])
    ...
    # --- Payment Options (2-Column Layout) ---
    story.append(Spacer(1, 5*mm))
    ...
    # Load QR code image for Option 2
    qr_path = Path(__file__).parent / "static" / "venmo.png"
    qr_img = None
    if qr_path.exists():
        try:
    ...
    # RIGHT SECTION: Option 2 with QR - matching quote PDF format
    right_section = [
        Table(
            [[Paragraph("<b>Option 2: Zelle QR code</b>", InvLabel)]],
            colWidths=[doc.width * 0.40],
            rowHeights=[8*mm],
            style=TableStyle([
    ...
    right_section.append(
        Paragraph("Scan to pay with Zelle", TableCenter)
    )

    # LEFT SECTION: Options 1 and 3
    left_section = [
        Table(
            [[Paragraph("<b>Option 1: Check</b>", InvLabel)]],
            colWidths=[doc.width * 0.55],
            rowHeights=[7*mm],
            style=TableStyle([
        ...
        Spacer(1, 4*mm),
        Table(
            [[Paragraph("<b>Option 3: Bank ACH Transfer</b>", InvLabel)]],
            colWidths=[doc.width * 0.55],
            rowHeights=[7*mm],
            style=TableStyle([
    ...
    payment_opts = [[left_section, right_section]]
    payment_table = Table(payment_opts, colWidths=[doc.width * 0.55, doc.width * 0.40])
    ...
    invoice_status = calculated_status.lower() if calculated_status else ''
    if invoice_status == 'paid':
        def add_paid_watermark(canvas_obj, doc_obj):
            ...
            # Draw PAID text - green, rotated, large with higher opacity

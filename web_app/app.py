...
def projects():
    ...
    # Set _plant_display and _has_co for each project
    for item in items:
        item["_plant_display"] = item.get("plant", "")
        # Check if project has change orders
        item["_has_co"] = bool(item.get("change_orders") and len(item.get("change_orders", {})) > 0)
    ...
    if plant_filter:
        items = [i for i in items if i.get("plant", "") == plant_filter]
    if date_from:
    ...
    clients = _load_clients()
    all_plants = sorted({p.get("plant", "") for p in items if p.get("plant", "")})
    ...
    return render_template("projects.html", projects=items, statuses=statuses,
                           ...
                           clients=clients, all_plants=all_plants,
...
def projects_export_pdf():
    ...
    elems.append(Spacer(1, 0.2*inch))
    hdrs = ["Project #", "Name", "Client", "Start Date", "End Date", "Status", "Contract Value", "Paid", "Outstanding"]
    data = [hdrs]
    def fmt_date_pdf(d):
        if not d or d == "—":
            return "—"
    ...
    cw = [1.3*inch, 2.5*inch, 2.0*inch, 1.2*inch, 1.2*inch, 1.2*inch, 1.3*inch, 1.3*inch, 1.3*inch]
    tbl = Table(data, colWidths=cw, repeatRows=1)
    tbl.setStyle(TableStyle([
        ("BACKGROUND",    (0,0), (-1,0), colors.HexColor("#0F172A")),
        ("TEXTCOLOR",     (0,0), (-1,0), colors.white),

...
def project_detail(project_id):
    ...
    # Load invoices linked to this project
    raw_inv = fb_get("/invoices") or {}
    project_invoices = []
    if isinstance(raw_inv, dict):
        for iid, idata in raw_inv.items():
            if isinstance(idata, dict):
                m = idata.get("meta", {})
                if m.get("project_number", "") == proj_num:
                    idata["firebase_id"] = iid
    ...
    # P&L totals
    inv_total   = sum(_safe_float(i.get("meta",{}).get("total", 0))      for i in project_invoices)
    inv_paid    = sum(_safe_float(i.get("meta",{}).get("amount_paid", 0)) for i in project_invoices)
...
def invoicing():
    ...
    return render_template("invoicing.html", invoices=items, statuses=statuses,
                           search=search, status_filter=status_filter,
                           clients=clients, projects=projects_list,
                           next_num=next_num, active_tab=active_tab,
...
def invoice_edit(invoice_id):
    ...
    if request.method == "POST":
        ...
        proj_num = updated["meta"].get("project_number", "")
        if proj_num:
...
def _sync_project_payment(project_number: str) -> None:
    """Sum all paid invoice amounts for a project and write back to project.amount_paid."""
    ...
    if isinstance(raw_inv, dict):
        for inv in raw_inv.values():
            if isinstance(inv, dict):
                m = inv.get("meta", {})
                if m.get("project_number", "") == project_number:
                    total_paid += _safe_float(m.get("amount_paid", 0))
    raw_proj = fb_get("/projects") or {}
...
def _auto_complete_project_if_paid(project_number: str) -> None:
    ...
    linked = [v.get("meta", {}) for v in raw_inv.values()
              if isinstance(v, dict) and v.get("meta", {}).get("project_number", "") == project_number]
    if not linked:
        return

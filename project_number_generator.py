...
def is_project_fully_paid(project: dict) -> bool:
    """Check if a project has been fully paid (100% completion)"""
    try:
        ...
        if not tracker:
            return False
        summary = tracker.get_payment_summary(project.get("project_number", ""), total_amount)
        return summary.get("payment_percentage", 0) >= 100.0
    except Exception as e:
        _log.error(f"Error checking if project is fully paid: {e}")
        return False
...
class ProjectNumberGeneratorTab(QtWidgets.QWidget):
    ...
    def __init__(self, main_window):
        ...
        self._view_mode = "active"      # "active" | "completed" | "all"
    ...
    def init_ui(self):
        ...
        main_layout.setContentsMargins(16, 14, 16, 18)
    ...
    def create_stats_section(self, layout):
        """Create a simple project command bar."""
        ...
        self.export_btn.setFixedSize(130, 44)
        ...
        section_title = QtWidgets.QLabel("Project Dashboard")
        ...
        """)
        section_hint = QtWidgets.QLabel("Create projects, review status, and send selected jobs to invoice.")
        ...
        # Create stat cards
        total_card = self.create_stat_card("Total Projects", "0", "#2563eb")
        self.total_projects_label = total_card.findChild(QtWidgets.QLabel)
        cards_layout.addWidget(total_card)

        cancelled_card = self.create_stat_card("Cancelled", "0", "#b45309")
        self.cancelled_label = cancelled_card.findChild(QtWidgets.QLabel)
        cards_layout.addWidget(cancelled_card)

        completed_card = self.create_stat_card("Completed", "0", "#00756f")
        self.completed_label = completed_card.findChild(QtWidgets.QLabel)
        cards_layout.addWidget(completed_card)

        main_layout.addStretch()
        main_layout.addLayout(cards_layout)
        total_card.setVisible(True)
        cancelled_card.setVisible(True)
        completed_card.setVisible(True)

        # Right side: Generate Project Button
        self.generate_project_btn = QtWidgets.QPushButton("➕ Add Project")
        self.generate_project_btn.setMinimumHeight(44)
        ...
        """)
        self.generate_project_btn.setText("New Project")
        self.generate_project_btn.setFixedSize(190, 44)
    ...
    def create_stat_card(self, title, value, color):
        """Create a stat card matching JobFormTab style"""
        card = QtWidgets.QWidget()
        card.setFixedSize(168, 76)
    ...
    def create_projects_table_section(self, layout):
        """Create projects table section matching JobFormTab style"""
        table_group = QtWidgets.QGroupBox("📊 Projects Overview")
        self.table_group = table_group
        table_group.setTitle("Project List")
        table_group.setStyleSheet(self.get_group_box_style())
        table_layout = QtWidgets.QVBoxLayout(table_group)
        table_layout.setContentsMargins(22, 24, 22, 22)
        table_layout.setSpacing(12)

        # ── Summary Cards Row ────────────────────────────────────────────────
        self._build_summary_cards(table_layout)

        # ── View Mode Tabs + Quick Period Filters ────────────────────────────
        self._build_view_tabs(table_layout)
        ...
        search_filter_layout.setContentsMargins(12, 10, 12, 10)

        # ===== Date Range Filter Button (LEFT SIDE) =====
        self.date_range_button = QtWidgets.QPushButton("📅")
        self.date_range_button.setText("Date")
        self.date_range_button.setFixedSize(72, 38)
        ...
        self.search_edit.setPlaceholderText("🔍 Search by project number, name, company, payment category, amount...")
        self.search_edit.setPlaceholderText("Search by project number, name, client, amount...")
        ...
        status_label.setStyleSheet("font-weight: 800; color: #0f172a; font-size: 13px; padding: 5px 0px;")
        ...
        # Client filter button (3-dots menu style)
        ...
        self.client_filter_button.setMaximumWidth(380)
        ...
        self.results_label.setMinimumWidth(220)
        ...
        self.projects_table.setHorizontalHeaderLabels([
            ...
            "Remaining", "Payment", "Start Date", "Due Date", "Status", "Actions"
        ])
        
        # Professional table styling with vertical grid lines
        ...
        self.projects_table.setMaximumHeight(560)
        ...
        self.projects_table.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarAsNeeded)
        ...
        self.projects_table.setTextElideMode(QtCore.Qt.ElideNone)
        ...
        # All columns fixed width — right-side columns always fully visible
        ...
        self.projects_table.setColumnHidden(self.COL_DUE_DATE,   True)

        # Right-side columns are Fixed — always fully visible
        # Project Name + Client both stretch to share remaining space equally
        self.projects_table.setColumnWidth(self.COL_PROJECT_NUMBER, 170)
        self.projects_table.setColumnWidth(self.COL_PROJECT_NAME,   100)  # min hint (stretch)
        self.projects_table.setColumnWidth(self.COL_CLIENT,         100)  # min hint (stretch)
        self.projects_table.setColumnWidth(self.COL_PLANT,           64)
        self.projects_table.setColumnWidth(self.COL_SALES,           72)
        self.projects_table.setColumnWidth(self.COL_PAYMENT,        230)
        self.projects_table.setColumnWidth(self.COL_STATUS,         175)
        self.projects_table.setColumnWidth(self.COL_ACTIONS,        125)

        # Project Name AND Client both stretch equally; all right columns locked
        header.setSectionResizeMode(self.COL_PROJECT_NAME,   QtWidgets.QHeaderView.Stretch)
        header.setSectionResizeMode(self.COL_CLIENT,         QtWidgets.QHeaderView.Stretch)
        header.setSectionResizeMode(self.COL_PROJECT_NUMBER, QtWidgets.QHeaderView.Fixed)
        header.setSectionResizeMode(self.COL_PLANT,          QtWidgets.QHeaderView.Fixed)
    ...
    def adjust_projects_table_height(self):
        ...
        target_height = max(132, min(target_height, 560))
    ...
    def _workspace_table_item(self, value, *, align=QtCore.Qt.AlignCenter, bold=False, color="#0f172a"):
        ...
        item.setFont(QtGui.QFont("Inter", 12, QtGui.QFont.DemiBold if bold else QtGui.QFont.Normal))
        return item
    ...
    def _stage_matches(self, left, right):
        ...
        if not left or not right:
            return False
        normalized = {
            "payment 1": ("payment 1", "term 1", "deposit", "down payment", "1st", "first"),
            "1st installment": ("payment 1", "term 1", "deposit", "down payment", "1st", "first"),
            "payment 2": ("payment 2", "term 2", "2nd", "second"),
            "2nd installment": ("payment 2", "term 2", "2nd", "second"),
            "payment 3": ("payment 3", "term 3", "3rd", "third"),
            "3rd installment": ("payment 3", "term 3", "3rd", "third"),
            "final payment": ("final", "payment 4", "term 4", "4th", "balance"),
            "full amount": ("full", "due", "full amount"),
            "deposit": ("deposit", "down payment", "payment 1"),
        }
    ...
    def _payment_plan_rows_for_workspace(self, project_data):
        ...
        for planned in planned_rows:
            ...
            invoice_status = matched_invoices[0].get("status", "Invoice Created") if matched_invoices else "Not Invoiced"
            rows.append({
                "stage": stage,
                ...
                "paid": paid,
                "remaining": max(planned["amount"] - paid, 0),
            })

        unassigned = [payment for payment in payments if payment.payment_id not in matched_payment_ids]
    ...
    def _set_list_view_visible(self, visible):
        for attr in ("header_frame", "stats_frame", "table_group"):
    ...
    def show_project_workspace(self, project_data):
        ...
        back_btn.setFixedSize(76, 34)
        back_btn.setStyleSheet("""
            QPushButton { background:#f8fafc; color:#475569;
                border:1px solid #e2e8f0; border-radius:7px;
                font-weight:700; font-size:12px; }
            QPushButton:hover { background:#e2e8f0; color:#0f766e; }
        """)
        ...
        STATUS_STYLES = {
            ...
            "In Progress": ("#1e40af", "#dbeafe", "#93c5fd"),
            "Not Started": ("#475569", "#f1f5f9", "#cbd5e1"),
            "Cancelled":   ("#7f1d1d", "#fee2e2", "#fca5a5"),
            "On Hold":     ("#78350f", "#fef3c7", "#fcd34d"),
            "Completed":   ("#065f46", "#dcfce7", "#86efac"),
        }
        ss = STATUS_STYLES.get(status, ("#475569", "#f1f5f9", "#cbd5e1"))
        status_pill = QtWidgets.QLabel(f"  {status}  ")
        status_pill.setAlignment(QtCore.Qt.AlignCenter)
        status_pill.setStyleSheet(f"""
            color:{ss[0]}; background:{ss[1]}; border:1px solid {ss[2]};
            border-radius:12px; font-size:12px; font-weight:700; padding:5px 14px;
        """)
        # Store reference so table status changes can update it
        self._workspace_status_pill = status_pill
        self._workspace_project_data = project_data
        h_lay.addWidget(status_pill)
        ...
        def _card(label, value, value_color="#0f172a"):
            ...
            val.setStyleSheet(f"font-size:22px; font-weight:800; color:{value_color};")
            lay.addWidget(lbl)
            lay.addWidget(val)
            return f
        ...
        pf_lbl.setStyleSheet("font-size:11px; font-weight:600; color:#94a3b8;")
        pf_val = QtWidgets.QLabel(f"{pct}% Paid")
        pf_val.setStyleSheet("font-size:22px; font-weight:800; color:#0f766e;")
        ...
        cards_row.addWidget(_card("Payment Plan",
            project_data.get("payment_terms","Single Payment") or "Single Payment"))
    ...
    def _open_workspace_payment_history(self, project_data):
        ...
        # Refresh workspace immediately so updated amounts are visible without close/reopen
        QtCore.QTimer.singleShot(0, lambda: self.show_project_workspace(project_data))
    ...
    def _create_workspace_overview_tab(self, project_data):
        ...
        for idx, (lbl, val) in enumerate(info_fields):
            ...
            val_w.setStyleSheet("font-size:13px; font-weight:700; color:#0f172a;")
    ...
    def _create_workspace_payments_tab(self, project_data):
        ...
        self._prepare_workspace_table(table)
        payments = sorted(
            get_payment_tracker().get_project_payments(project_data.get("project_number", "")),
            ...
            for row_index, payment in enumerate(payments):
                ...
                for col, value in enumerate(values):
                    if col == 3:
                        ...
                        align = QtCore.Qt.AlignLeft if col in (0, 5) else QtCore.Qt.AlignCenter
    ...
    def _set_project_item_readability(self, item, text="", *, emphasis=False, muted=False, left=False):
        ...
        item.setTextAlignment(
            ...
            if left else QtCore.Qt.AlignCenter
        )
        font = QtGui.QFont("Inter", 12)
    ...
    def _style_project_number_cell(self, item, project):
        ...
        item.setToolTip(f"Click to view all details for {project_number}")
        font = QtGui.QFont("Consolas", 12)   # monospace → no wrapping on numbers
    ...
    def load_projects_to_invoice_direct(self, selected_project_objects):
        ...
        try:
            ...
            for project in selected_project_objects:
                ...
                if last_row:
                    last_row.down_payment_combo.setCurrentText(invoice_payment_category)
                    if hasattr(last_row, "update_total"):
                        last_row.update_total()
    ...
    def update_stats(self):
        ...
        for project in self.generated_projects:
            ...
            matches_status = (
                ...
                project.get('status', '') == status_filter
            )
            ...
            if matches_client and matches_search and matches_status and matches_date:
    ...
    def update_projects_table(self, projects_list=None):
        ...
        for row, project in enumerate(valid_projects):
            ...
            sno_item.setFont(QtGui.QFont("Inter", 12))
            sno_item.setBackground(bg_color)
            self.projects_table.setItem(row, 0, sno_item)
            
            # Project Number
            project_item = QtWidgets.QTableWidgetItem(project["project_number"])
            project_item.setTextAlignment(QtCore.Qt.AlignCenter)
            project_item.setData(QtCore.Qt.UserRole, project)
            project_item.setForeground(QtGui.QColor('#2c3e50'))
            project_item.setFont(QtGui.QFont("Inter", 12))
            project_item.setBackground(bg_color)
            self._set_project_item_readability(project_item, project.get("project_number", ""), emphasis=True)
            self._style_project_number_cell(project_item, project)
            ...
            company_item.setForeground(QtGui.QColor('#34495e'))
            company_item.setFont(QtGui.QFont("Inter", 12))
            company_item.setBackground(bg_color)
            self._set_project_item_readability(company_item, project.get("company", ""), left=True)
            ...
            po_wo_item.setFont(QtGui.QFont("Inter", 12))
            ...
            date_received_item.setForeground(QtGui.QColor('#7f8c8d'))
            date_received_item.setFont(QtGui.QFont("Inter", 11))
            date_received_item.setBackground(bg_color)
            self._set_project_item_readability(date_received_item, project.get("date_received", ""), muted=True)
            ...
            plant_item.setFont(QtGui.QFont("Inter", 12))
            ...
            # Sales
            sales_item = QtWidgets.QTableWidgetItem(project.get("sales", ""))
            sales_item.setTextAlignment(QtCore.Qt.AlignCenter)
            sales_item.setForeground(QtGui.QColor('#2c3e50'))
            sales_item.setFont(QtGui.QFont("Inter", 12))
            sales_item.setBackground(bg_color)
            self._set_project_item_readability(sales_item, project.get("sales", ""), left=True)
            self.projects_table.setItem(row, 7, sales_item)
            ...
            amount_item.setFont(QtGui.QFont("Inter", 12, QtGui.QFont.DemiBold))
            ...
            start_date_item.setForeground(QtGui.QColor('#7f8c8d'))
            start_date_item.setFont(QtGui.QFont("Inter", 11))
            start_date_item.setBackground(bg_color)
            self._set_project_item_readability(start_date_item, project.get("start_date", ""), muted=True)
            ...
            due_date_item.setForeground(QtGui.QColor('#7f8c8d'))
            due_date_item.setFont(QtGui.QFont("Inter", 11))
            due_date_item.setBackground(bg_color)
            self._set_project_item_readability(due_date_item, project.get("due_date", ""), muted=True)
            self.projects_table.setItem(row, self.COL_DUE_DATE, due_date_item)
            
            # Status dropdown — auto-paid lock if fully paid
            status_combo = self._build_status_combo(project)
            self.projects_table.setCellWidget(row, self.COL_STATUS, status_combo)
            
            # Enhanced action buttons
            self.add_enhanced_action_buttons(row, project)
        
        for row in range(self.projects_table.rowCount()):
            self.projects_table.setRowHeight(row, 78)
        self.projects_table.verticalHeader().setDefaultSectionSize(78)
        if hasattr(self.projects_table, "refresh_frozen_project_column"):
            self.projects_table.refresh_frozen_project_column(118)
    ...
    def style_project_status_combo(self, combo: QtWidgets.QComboBox, status: str):
        ...
        combo.setToolTip(f"Status: {status}  ·  Click to change")
        combo.setEditable(False)
    
    def _build_status_combo(self, project: dict) -> QtWidgets.QComboBox:
        """Build a styled status combo, auto-set to Paid and lock it if fully paid."""
        ...
        is_fully_paid = pct >= 100.0 and total > 0

        # Auto-promote status to Paid if 100% received
        if is_fully_paid and project.get("status") not in ("Paid", "Cancelled"):
            project["status"] = "Paid"
            # Persist silently to Firebase
            try:
                from firebase_admin import db as _db
                data = _db.reference('/projects').order_by_child('project_number').equal_to(pn).get()
                if data:
                    pid = list(data.keys())[0]
                    _db.reference(f'/projects/{pid}').update({
                        'status': 'Paid',
        ...
        combo.setMinimumWidth(134)
        combo.setFixedHeight(32)

        if is_fully_paid:
            # Lock the dropdown — project is fully paid
            combo.setEnabled(False)
            combo.setToolTip("Project fully paid — status locked as Paid")
        else:
    ...
    def on_project_status_changed(self, project_data: dict, new_status: str, combo: QtWidgets.QComboBox):
        ...
        try:
            ...
            # Sync status pill in the open workspace window if it shows the same project
            pill = getattr(self, '_workspace_status_pill', None)
            ws_data = getattr(self, '_workspace_project_data', None)
            if pill and ws_data and ws_data.get('project_number') == project_data.get('project_number'):
                STATUS_STYLES = {
                    "Paid":        ("#065f46", "#d1fae5", "#6ee7b7"),
                    "In Progress": ("#1e40af", "#dbeafe", "#93c5fd"),
                    "Not Started": ("#475569", "#f1f5f9", "#cbd5e1"),
                ...
                ss = STATUS_STYLES.get(new_status, ("#475569", "#f1f5f9", "#cbd5e1"))
                pill.setText(f"  {new_status}  ")
    ...
    def add_enhanced_action_buttons(self, row, project):
        """Add a clean Actions button that shows a popup menu on click."""
        actions_widget = QtWidgets.QWidget()
        actions_layout = QtWidgets.QHBoxLayout(actions_widget)
        actions_layout.setContentsMargins(6, 6, 6, 6)
        actions_layout.setAlignment(QtCore.Qt.AlignCenter)

        # Use QPushButton (not QToolButton) so no dropdown arrow appears
        actions_btn = QtWidgets.QPushButton("⋮  Actions")
        actions_btn.setFixedSize(96, 34)
        ...
        """)
        view_action = menu.addAction("View Project")
        view_action.triggered.connect(lambda checked=False: self.show_project_workspace(project))
        edit_action = menu.addAction("Edit Project")
        edit_action.triggered.connect(lambda checked=False: self.edit_single_project(project))
        ...
        delete_action.triggered.connect(lambda checked=False: self.delete_single_project(project))

        # Show menu below the button on click
        actions_btn.clicked.connect(
            lambda: menu.exec_(actions_btn.mapToGlobal(
                QtCore.QPoint(0, actions_btn.height() + 2)))
        )
        actions_layout.addWidget(actions_btn)
        
        # Set the widget in the table and increase row height
    ...
    def _on_payment_changed(self, project_number: str):
        ...
        for row in range(self.projects_table.rowCount()):
            ...
            if project_data and project_data.get("project_number") == project_number:
                bg_color = QtGui.QColor("#ffffff" if row % 2 == 0 else "#f8f9fa")
                ...
                break
        # Also refresh all finance tabs in real-time
        self._refresh_finance_tabs()
    ...
    def _do_refresh_finance_tabs(self):
        """Actually refresh the finance tabs (called once after debounce settles)."""
        ...
        for attr in ("balance_sheet_tab", "expenses_tab", "finance_overview_tab"):
            ...
            if tab and hasattr(tab, "refresh_data"):
                try:
                    ...
                    _log.warning("Could not refresh %s: %s", attr, e)
        ...
        for invoice_data in invoices:
            ...
            for item in invoice_data.get("items", []):
                ...
                rows.append({
                    ...
                    "amount": self._format_project_money(amount),
                    "status": status,
    ...
    def _get_project_payment_schedule_rows(self, project_data: dict):
        ...
        for planned in planned_rows:
            ...
            if remaining <= 0 and planned_amount > 0:
                ...
                status = matched_invoice.get("status", "Invoice Created") or "Invoice Created"
            else:
                status = "Not Invoiced"
    ...
    def _create_paid_amount_item(self, project_data, bg_color):
        ...
        paid_item.setFont(QtGui.QFont("Inter", 12, QtGui.QFont.DemiBold))
    ...
    def _create_remaining_due_item(self, project_data, bg_color):
        ...
        remaining_font = QtGui.QFont("Inter", 12, QtGui.QFont.DemiBold)
    ...
    def view_project_details(self, project_data):
        ...
        details = [
            ...
            ("Plant:", project_data.get("plant", "")),
            ("Sales:", project_data.get("sales", "")),
            ("Project Amount:", f"${project_data.get('project_amount', 0):,.2f}"),
            ("Payment Category:", project_data.get("payment_category", "")),
    ...
    def apply_client_menu_filter(self, company_name):
        ...
        if company_name == "All Clients" or company_name == "📂 All Clients":
            self.client_filter_button.setText("🗂️")
            self.client_filter_button.setStyleSheet("""
                QPushButton {
                    background: transparent;
                    border: none;
    ...
    def filter_projects(self):
        ...
        for project in self.generated_projects:
            # ── View-mode filter ────────────────────────────────────────
            if not _view_matches(project.get("status", "")):
                continue
        ...
        for row, project in enumerate(filtered_projects):
            ...
            date_received_item.setTextAlignment(QtCore.Qt.AlignCenter)
            date_received_item.setForeground(QtGui.QColor('#7f8c8d'))
            date_received_item.setFont(QtGui.QFont("Inter", 12))
            date_received_item.setBackground(bg_color)
            self._set_project_item_readability(date_received_item, project.get("date_received", ""), muted=True)
            self.projects_table.setItem(row, 5, date_received_item)
            ...
            if amount != 0.0:
                amount_item.setForeground(QtGui.QColor('#27ae60'))
                amount_item.setFont(QtGui.QFont("Inter", 12))
            else:
                amount_item.setForeground(QtGui.QColor('#95a5a6'))
            amount_item.setBackground(bg_color)
            amount_item.setToolTip(Currency.format(amount))
            ...
            start_date_item.setTextAlignment(QtCore.Qt.AlignCenter)
            start_date_item.setForeground(QtGui.QColor('#7f8c8d'))
            start_date_item.setFont(QtGui.QFont("Inter", 12))
            start_date_item.setBackground(bg_color)
            self._set_project_item_readability(start_date_item, project.get("start_date", ""), muted=True)
            self.projects_table.setItem(row, self.COL_START_DATE, start_date_item)
            ...
            due_date_item.setTextAlignment(QtCore.Qt.AlignCenter)
            due_date_item.setForeground(QtGui.QColor('#7f8c8d'))
            due_date_item.setFont(QtGui.QFont("Inter", 12))
            due_date_item.setBackground(bg_color)
            self._set_project_item_readability(due_date_item, project.get("due_date", ""), muted=True)
            self.projects_table.setItem(row, self.COL_DUE_DATE, due_date_item)
            ...
            self.style_project_status_combo(status_combo, current_status)
            status_combo.setMinimumWidth(134)
            status_combo.setFixedHeight(32)
            status_combo.currentTextChanged.connect(
                lambda status, project_data=project, combo=status_combo: 
                self.on_project_status_changed(project_data, status, combo)
            )
    ...
    def _create_payment_cell_widget(self, project_data):
        """Payment column: one clear card showing current stage + paid/remaining."""
        container = QtWidgets.QWidget()
        ...
        # Find the stage AFTER the current one (for "Next:" preview)
        after_stage_name = ""
        planned_rows = self._planned_payment_rows(project_data)
            ...
            for i, pr in enumerate(planned_rows):
                if pr.get("stage") == cur_stage_name:
                    if i + 1 < len(planned_rows):
                        after_stage_name = planned_rows[i + 1].get("stage", "")
                    else:
                        after_stage_name = "— Final Stage"   # last stage label
                    break
        ...
        stage_lbl.setMinimumWidth(200)
        stage_lbl.setFixedHeight(26)
        ...
        if after_stage_name and after_stage_name != "— Final Stage":
            next_text  = f"Next → {after_stage_name}"
            next_color = "#64748b"
        elif after_stage_name == "— Final Stage":
            next_text  = "Final payment stage"
            next_color = "#b45309"
        elif total_stages == 1 and pct < 100:
            next_text  = "Single payment plan"
            next_color = "#94a3b8"
        else:
            next_text = ""
            next_color = "#94a3b8"
    ...
    def delete_single_project(self, project):
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            ...
            # Cascade: remove payment tracker records and their balance sheet entries
            self._cascade_delete_project_data(project_number)
            ...
            QtWidgets.QMessageBox.information(
                ...
                f"Associated payments and financial records removed."
            )
    ...
    def _cascade_delete_project_data(self, project_number: str):
        """Remove all payment tracker records and balance sheet revenue entries for a project."""
        try:
            from payment_tracker import get_payment_tracker
            tracker = get_payment_tracker()
            project_payments = tracker.get_project_payments(project_number)
            for payment in list(project_payments):
                tracker.delete_payment(payment.payment_id)
            if project_payments:
                _log.info("Cascade-deleted %d payment(s) for project %s",
                          len(project_payments), project_number)

            # Also remove revenue entries linked to this project from Firebase
            if FIREBASE_AVAILABLE:
                try:
                    from firebase_admin import db as _db
                    ref = _db.reference('revenue')
                    all_revenue = ref.get() or {}
                    for rev_id, rev in all_revenue.items():
                        if isinstance(rev, dict) and rev.get("project_number") == project_number \
                                and rev.get("is_payment"):
                            ref.child(rev_id).delete()
                            _log.info("Cascade-deleted revenue entry %s for project %s",
                                      rev_id, project_number)
                except Exception as e:
                    _log.warning("Could not cascade-delete revenue entries: %s", e)
        except Exception as e:
            _log.warning("Error in cascade delete for project %s: %s", project_number, e)
    ...
    def generate_projects_combined_pdf(self, projects, export_params):
        ...
        try:
            ...
            # Header with MABS Engineering LLC centered
            page_width = landscape(A4)[0]
            header_data = [[Paragraph("MABS Engineering LLC", mabs_header_style)]]
            ...
            if projects:
                ...
                col_widths = [
                    0.35 * inch,   # S.No.
                    0.95 * inch,   # Project Number (increased slightly for "Project #" to "Project Number")
                ...
                table_data = [[
                    "S.No.", "Project Number", "Project Name", "Client", "PO/WO",
                    "Received Date", "Plant", "Sales", "Price", "Payment Category",
                ...
                for idx, project in enumerate(projects, 1):
                    ...
                    table_data.append([
                        str(idx),
                        truncate(project.get('project_number', ''), 12),
                        truncate(project.get('project_name', ''), 18),
                        truncate(project.get('company', ''), 16),
                        truncate(project.get('po_wo_number', ''), 12),
                        date_received_us,
                        truncate(project.get('plant', ''), 10),
                        truncate(project.get('sales', ''), 10),
                        amount_formatted,
                        truncate(payment_category, 14),
                        start_date_us,
                        due_date_us,
                        truncate(status_value, 18)
                    ])
    ...
    def generate_projects_combined_excel(self, projects, export_params):
        ...
        try:
            ...
            ws['A1'] = "MABS ENGINEERING LLC - PROJECTS REPORT"
            ...
            headers = [
                ...
                "Sales", "Payment Category", "Start Date", "Due Date", "Status", "Notes"
            ]
...
class ProjectDialog(QtWidgets.QDialog):
    ...
    def init_ui(self):
        ...
        self.job_number_edit = self.create_styled_line_edit("Enter Quote Number (Optional)")
        self.job_number_edit.setFixedWidth(200)
        ...
        # Custom % input (shown only for custom_down)
        self._custom_pct_row = QtWidgets.QHBoxLayout()
        custom_lbl = QtWidgets.QLabel("Down Payment %:")
        custom_lbl.setStyleSheet("font-size:13px;font-weight:700;color:#334155;min-width:130px;")
        self._custom_pct_row.addWidget(custom_lbl)
        ...
        self.custom_pct_spin.setFixedWidth(100)
        self.custom_pct_spin.setFixedHeight(36)
        self.custom_pct_spin.setStyleSheet(
            "QSpinBox{padding:6px 10px;border:1.5px solid #d8e2ec;border-radius:7px;"
            "font-size:13px;font-weight:600;background:white;color:#0f172a;"
            "font-family:'Inter','Segoe UI';}"
            "QSpinBox:focus{border-color:#0f766e;}"
        )
        self._custom_pct_row.addWidget(self.custom_pct_spin)
        self._custom_pct_row.addStretch()
        self._custom_pct_widget = QtWidgets.QWidget()
        self._custom_pct_widget.setLayout(self._custom_pct_row)
        ...
        # Payment summary label (live preview)
        ...
        pay_vbox.addWidget(self._pay_summary_lbl)
    ...
    def populate_form_data(self):
        ...
        # Quote Number
        self.job_number_edit.setText(self.project_data.get('job_number', ''))
    ...
    def collect_project_data(self):
        ...
        down_pct = 0.0 if ptype == "no_down" else (50.0 if ptype == "50_down" else float(pct))
    ...
    def load_job_details(self):
        ...
        try:
            ...
            if job_data:
                ...
                # Price from Engineering Costs
                cost_text = job_data.get('engineering_costs', '0')
                if cost_text:
                    try:
                        cost_text = cost_text.replace('$', '').replace(',', '').strip()

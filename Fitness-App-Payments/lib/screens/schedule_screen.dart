...
class _MySchedulePageState extends State<MySchedulePage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Mark past sessions as completed when the page loads
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _markPastSessionsAsCompleted();
    });
    
    // Also check every minute for newly completed sessions
    ...
    _firestore
      ...
      try {
        ...
        for (var doc in snapshot.docs) {
          ...
          String perUserStatus = statusByUser[user.uid] as String? ?? 'Confirmed';
          
          // ✅ FIX: Auto-detect past sessions even if not marked in DB
          // This ensures proper display even before the batch update runs
          if (perUserStatus != 'Cancelled' && perUserStatus != 'Completed') {
            // Check if session end time has passed
            bool isPastSession = false;
            try {
              final endTimeStr = timeString.split(' - ')[1].trim();
          ...
          final slot = {
            ...
            'status': perUserStatus,
            'docRef': doc.reference,
          };

          // Check if session is actually completed (either marked or time passed)
          bool isActuallyCompleted = perUserStatus == 'Completed';
          bool isActuallyCancelled = perUserStatus == 'Cancelled';
          
          // For sorting past/completed vs upcoming
          bool isPastOrCompleted = false;
          if (perUserStatus == 'Completed' || perUserStatus == 'Cancelled') {
            ...
            try {
              ...
              isPastOrCompleted = now.isAfter(slotEndTime);
            } catch (_) {
              isPastOrCompleted = slotDateTime.isBefore(now);
            }
          }
  ...
  // Add this method to mark past sessions as completed
  Future<void> _markPastSessionsAsCompleted() async {
    ...
    try {
      // Get all slots where this user is booked
      ...
      for (final slotDoc in slotsSnapshot.docs) {
        ...
        final currentStatus = statusByUser[user.uid] as String?;
        ...
        if (slotTime.isNotEmpty) {
          try {
            // Parse the end time (e.g., "5:30 AM - 6:30 AM" -> "6:30 AM")
            ...
            // If parsing fails, check if the date has passed
        ...
        // If the session is past, mark it as Completed
        if (isPast && currentStatus != 'Completed') {
          batch.update(slotDoc.reference, {
            'status_by_user.${user.uid}': 'Completed',
            'last_updated': FieldValue.serverTimestamp(),
          });
          hasUpdates = true;
          print('✅ Marking session as Completed: ${slotData['time']} on ${slotDate.toLocal()}');
        }
      }
      
      if (hasUpdates) {
        await batch.commit();
        print('✅ Successfully marked past sessions as Completed');
      }
    } catch (e) {

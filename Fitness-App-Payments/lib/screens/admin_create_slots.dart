...
class _AdminCreateSlotsScreenState extends State<AdminCreateSlotsScreen> {
  ...
  DateTime _parseSlotEndTime(String slotTime, DateTime slotDate) {
    try {
      ...
      final endTimeStr = slotTime.split(' - ')[1].trim().toUpperCase();
      DateTime endTime;
      try {
        endTime = DateFormat("h:mm a", 'en_US').parseStrict(endTimeStr);
      } catch (_) {
        endTime = DateFormat("hh:mm a", 'en_US').parseStrict(endTimeStr);
      }
      ...
      return slotDate.add(const Duration(hours: 1));
    }
  }
  ...
  DateTime _parseSlotDateTime(String slotTime, DateTime baseDate) {
    try {
      ...
      final startTimeStr = slotTime.split(' - ')[0].trim().toUpperCase();
      DateTime startTime;
      try {
        startTime = DateFormat("h:mm a", 'en_US').parseStrict(startTimeStr);
      } catch (_) {
        startTime = DateFormat("hh:mm a", 'en_US').parseStrict(startTimeStr);
      }
      // Create DateTime in LOCAL time (not UTC)
      return DateTime(
        baseDate.year,
